...
class ByePlugin(Star):
    ...
    def _get_cfg(self, group: str, key: str, default):
        if (
            group in self.config
            and isinstance(self.config[group], dict)
            and key in self.config[group]
        ):
    ...
    async def on_group_notice(self, event: AiocqhttpMessageEvent):
        ...
        if raw_message.get("notice_type") == "group_ban":
            ...
            try:
                max_mute_duration = float(
                    self._get_cfg("mute_trigger", "max_mute_duration", 0.0)
                )
            except Exception:
                max_mute_duration = 0.0

            use_expected_raw = self._get_cfg(
                "mute_trigger", "use_expected_mute_duration_for_leave", "expected"
            )
            use_expected = (
                use_expected_raw
                if isinstance(use_expected_raw, bool)
                else (use_expected_raw == "expected")
            )
            ...
            if duration > 0:
                ...
                future_dur = (
                    curr_dur if use_expected else curr_dur + (duration / 3600.0)
                )
                logger.info(
                    f"机器人在群 {group_id} 被禁言 {duration} 秒，当前累计次数: {curr_count}，当前已锁定时长: {curr_dur:.2f} 小时"
                )
                ...
                if should_leave:
                    leave_message = self._get_cfg(
                        "general", "leave_message", "看来这个群不欢迎我，退了退了"
                    )
                    ...
                    async def execute_leave(target_gid_str, wait_sec, msg):
                        ...
                        try:
                            await event.bot.send_group_msg(
                                group_id=int(target_gid_str), message=msg
                            )
                        except Exception as e:
                            logger.error(
                                f"被禁言退群前发送遗言失败（可能是权限不足）: {e}"
                            )
                        try:
                            await event.bot.set_group_leave(
                                group_id=int(target_gid_str)
                            )
                    ...
                    asyncio.create_task(
                        execute_leave(gid_str, duration + 1, leave_message)
                    )
                else:
                    self._update_warning_card(
                        event, group_id, self_id, curr_count, future_dur
                    )
                ...
                if "current_ban_start" in self.mute_stats.get(gid_str, {}):
                    actual_duration_sec = (
                        time.time() - self.mute_stats[gid_str]["current_ban_start"]
                    )
                    del self.mute_stats[gid_str]["current_ban_start"]
                    if not use_expected:
                        self.mute_stats[gid_str]["duration"] += (
                ...
                curr_dur = self.mute_stats[gid_str]["duration"]
                logger.info(
                    f"群 {group_id} 禁言提前解除，当前累计次数: {curr_count}，已锁定实际时长: {curr_dur:.2f} 小时"
                )
                ...
                if should_leave:
                    ...
                    try:
                        await event.bot.send_group_msg(
                            group_id=int(group_id), message=leave_message
                        )
                    except Exception as e:
                    ...
                    if gid_str in self.pending_leaves:
                        logger.info(
                            f"提前解除禁言成功拯救此时的机器人，撤销原定退群计划: {group_id}"
                        )
                        del self.pending_leaves[gid_str]
                        await self.save_pending_leaves()
                    self._update_warning_card(
                        event, group_id, self_id, curr_count, curr_dur
                    )
    ...
    def _update_warning_card(self, event, group_id, self_id, curr_count, projected_dur):
        ...
        async def do_update():
            ...
            warning_dur = self._get_cfg(
                "mute_trigger", "warning_mute_duration_left", 10
            )
            max_mute_count = self._get_cfg("mute_trigger", "max_mute_count", 3)
            try:
                max_mute_duration = float(
            ...
            rem_dur_mins = (
                (max_mute_duration - projected_dur) * 60.0
                if max_mute_duration > 0
                else -1.0
            )
                ...
                if parts:
                    try:
                        info = await event.bot.get_group_member_info(
                            group_id=int(group_id), user_id=int(self_id)
                        )
                        ...
                        await event.bot.set_group_card(
                            group_id=int(group_id), user_id=int(self_id), card=new_card
                        )
                    except Exception as e:
                        logger.error(
                            f"被禁言后未能成功修改群名片以做出警告, 可能是权限等问题: {e}"
                        )
    ...
    async def check_pending_leaves_on_msg(self, event: AstrMessageEvent):
        ...
        for gid_str, data in list(self.pending_leaves.items()):
            ...
            if now >= data["run_at"]:
                logger.info(
                    f"检测到可能因重启中断的退群任务，尝试补发并退出: {gid_str}"
                )
                msg = data.get("message", "")
                try:
                    if msg:
                        await event.bot.send_group_msg(
                            group_id=int(gid_str), message=msg
                        )
                except Exception as e:
...
消息内容：{message_str}"""
        ...
        try:
            llm_resp = await self.context.llm_generate(
                chat_provider_id=provider_id if provider_id else None, prompt=prompt
            )

            if (
                not llm_resp
                or not hasattr(llm_resp, "completion_text")
                or not llm_resp.completion_text
            ):
                logger.error(
                    "大模型返回检测结果为空或格式不合法（无法解析 completion_text）。"
                )
                return
            ...
            if "是" in result:
                ...
                logger.info(
                    f"群 {group_id} 敌意发言计数 +1，当前累计: {curr_hostile}/{max_hostile}"
                )
                ...
                if curr_hostile >= max_hostile:
                    ...
                    logger.info(
                        f"敌意发言次数已达上限 {max_hostile} 次，准备执行退群: {group_id}"
                    )
        ...
        try:
            ...
            if not group_id:
                ...
                if yield_result:
                    ...
                    try:
                        await event.bot.send_msg(event.get_sender_id(), msg)
                    except:
                        pass
                return
